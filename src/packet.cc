    ...
    payload_len_()
{
  ...
  ack_number_ = str.substr( 1*sizeof( uint64_t ), sizeof( uint64_t ) );
  send_timestamp_ = str.substr( 2*sizeof( uint64_t ), sizeof( uint64_t ) );
  echo_reply_timestamp_ = str.substr( 3*sizeof( uint64_t ), sizeof( uint64_t ) );
  payload_len_ = str.substr( 4*sizeof(uint64_t), sizeof( uint64_t) );
  payload_ = str.substr( 5*sizeof( uint64_t ) );
}
